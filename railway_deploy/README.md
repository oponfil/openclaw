...
7. При отсутствии `--no-wait` ждёт готовности (статус `SUCCESS` и/или HTTP-пробы `/healthz`, `/`, `/openclaw`) с таймаутом до ~15 минут. Если Railway API для статусов нестабилен, скрипт переключается на HTTP-only проверку.
8. Выводит в лог **OPENCLAW_GATEWAY_TOKEN** и ссылку на Control UI (по умолчанию в корне домена).
